<?php 
    ...
    if( isset( $_POST['login'] ) ) {
        ...
        $doctor = $db->display( $sql, $params );
        if( $doctor ) {
            if( !$doctor[0]['verified'] ) {
                $message = '<div class="alert alert-danger alert-dismissible" role="alert">Sorry you are not approved yet.Please contact Administrator!</div>';
            } else {
                setcookie('userid', $doctor[0]['doctor_id'], time() + (86400 * 30), "/");
                setcookie('type', 'doctor', time() + (86400 * 30), "/");
                header('Location: dashboard.php');
                exit();
            }
        } else {
            $message = '<div class="alert alert-danger alert-dismissible" role="alert">Invalid username or password!</div>';
        }
    }
